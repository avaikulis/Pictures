...
public class ContainerEnderFurnace extends Container {
	...
	private TileEntityEnderFurnaceAndFuel tileFurnace;
	private int lastCookTime;
	private int lastBurnTime;
	private int lastItemBurnTime;
	
	public ContainerEnderFurnace(InventoryPlayer player, TileEntityEnderFurnaceAndFuel tileEntityFurnace){
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int par2){
		...
		if(slot != null && slot.getHasStack()){
			...
			if(par2 == 2){
				...
				if(EnderFurnaceRecipes.smelting().getSmeltingResult(itemstack1) != null){
					if(!this.mergeItemStack(itemstack1, 0, 1, false)){
						return null;
					}
				}else if(TileEntityEnderFurnaceAndFuel.isItemFuel(itemstack1)){
